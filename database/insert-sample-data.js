...
connection.connect((err) => {
    ...
    connection.query('SELECT id FROM products ORDER BY RAND() LIMIT 5', (err, products) => {
        ...
        connection.query('DELETE FROM favorites', () => {
            ...
            products.forEach((product, index) => {
                const userId = (index % 3) + 1; // 用户ID: 1, 2, 3
    ...
    function showResults() {
        ...
        connection.query('SELECT id, username, email FROM users', (err, users) => {
            ...
            // 查看收藏
            connection.query(`
                SELECT f.id, u.username, p.name as product_name, f.created_at 
                FROM favorites f 
            ...
            `, (err, favorites) => {
                if (!err) {
                    ...
                    favorites.forEach(fav => {
                        console.log(`  ID: ${fav.id}, 用户: ${fav.username}, 商品: ${fav.product_name}, 时间: ${fav.created_at}`);
                    });
                }
